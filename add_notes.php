		...
		?>
			<div class="card">
				...
				<div class="card-body">
					...
					$kode = $nama = $judul = $tgl = $keterangan = $barang = "";
					$no = $_GET["no"];
					$insert = '1';
					...
					<form class="form-horizontal" method="post" action="add_<?php echo $halaman; ?>" id="Myform">
								...
								<div class="col-md-12">
									<select class="form-control select2" style="width: 100%;" name="nama" required>
										...
										while ($row = mysqli_fetch_assoc($sql)) {
											if ($barang == $row['nama'])
												echo "<option value='" . $row['nama'] . "' selected='selected'>" . $row['nama'] . " | " . $row['nama'] . "|" .  $row['gudang'] . "</option>";
											else
												echo "<option value='" . $row['nama'] . "' >" . $row['nama'] . " | " . $row['nama'] . " | " . $row['gudang'] . "</option>";
										}
										?>
									</select>
								</div>
						...
						<div class="row mb-1">
							<label for="nama" class="col-sm-3">Keterangan</label>
							<textarea name="keterangan" placeholder="masukan keterangan notes"><?= $keterangan ?></textarea>
						</div>
						...
						if ($_SERVER["REQUEST_METHOD"] == "POST") {
							...
							if (mysqli_num_rows($result) > 0) {
								if ($chmod >= 3 || $_SESSION['jabatan'] == 'admin') {
									...
									// $sql = "update `notes` set nama=\'as\', judul=\'daa\', tgl=2020-04-30 ,keterangan=\'ada\' where kode=\'0001\'";
